...
size_t HNSWLibIndexOperations::parallelAddResetFirstCandidate(
    HierarchicalNSW<float>& index,
    const std::vector<std::vector<float>>& points, 
    const std::vector<size_t>& labels,
    const std::string& dataset_name,
    ...
    try {
        ...
        util::addResetFirstCandidateMultiThread(index, points, labels, num_threads, dataset_name, iteration, benchmarks, bench_mutex, repl_method_dist, repl_method_dist_mutex);
