...
HNSWLibExclusiveUPRunner(int iterations, int threads) : db(nullptr), con(db), max_iterations(iterations), threads(threads) {
    ...
    void runTest(int datasetIdx) {
        try {
            ...
            std::string output_dir = "hnswlib/results/unreachable_points_exclusive/" +  experiment + dataset.name + "_" + std::to_string(test_vectors_count) + "q_" + std::to_string(max_iterations) + "i_" + std::to_string(sample_size) + "r/";
...
int main() {
    ...
    try {
        ...
        // gist
        HNSWLibExclusiveUPRunner g_runner(max_iterations, executor_threads);
        g_runner.runTest(3);
