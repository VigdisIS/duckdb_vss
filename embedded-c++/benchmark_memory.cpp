...
void SetupIndex(Connection &con, const std::string& table_name) {

    CleanUpIndexes(con);

    std::cout << "Setting up index for " << table_name << std::endl;
    ...
    std::cout << "Index size: " << indexes_count_updated->GetValue(0, 0).ToString() << " Expected "  << std::to_string(1) << std::endl;
