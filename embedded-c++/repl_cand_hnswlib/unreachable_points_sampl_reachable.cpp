...
HNSWLibExclusiveUPRunner(int iterations, int threads, bool use_neigh_update, bool include_tombstones) : db(nullptr), con(db), max_iterations(iterations), threads(threads), use_neigh_update(use_neigh_update), include_tombstones(include_tombstones) {
    ...
    void runTest(int datasetIdx) {
        try {
            ...
            std::string output_dir = "repl_cand_hnswlib/results/" + alg_config + "/unreachable_points_exclusive/" +  experiment + dataset.name + "_" + std::to_string(test_vectors_count) + "q_" + std::to_string(max_iterations) + "i_" + std::to_string(sample_size) + "r/";
...
int main() {
    ...
    try {
        alg_config = "00";
        // fashion_mnist
        HNSWLibExclusiveUPRunner fm_00_runner(max_iterations, executor_threads, false, false);
        fm_00_runner.runTest(0);
        // mnist
        HNSWLibExclusiveUPRunner m_00_runner(max_iterations, executor_threads, false, false);
        m_00_runner.runTest(1);
        // sift
        HNSWLibExclusiveUPRunner s_00_runner(max_iterations, executor_threads, false, false);
        s_00_runner.runTest(2);
        // gist
        HNSWLibExclusiveUPRunner g_00_runner(max_iterations, executor_threads, false, false);
        g_00_runner.runTest(3);

        alg_config = "01";
        // fashion_mnist
        HNSWLibExclusiveUPRunner fm_01_runner(max_iterations, executor_threads, false, true);
        fm_01_runner.runTest(0);
        // mnist
        HNSWLibExclusiveUPRunner m_01_runner(max_iterations, executor_threads, false, true);
        m_01_runner.runTest(1);
        // sift
        HNSWLibExclusiveUPRunner s_01_runner(max_iterations, executor_threads, false, true);
        s_01_runner.runTest(2);
        // gist
        HNSWLibExclusiveUPRunner g_01_runner(max_iterations, executor_threads, false, true);
        g_01_runner.runTest(3);

        alg_config = "10";
        // fashion_mnist
        HNSWLibExclusiveUPRunner fm_10_runner(max_iterations, executor_threads, true, false);
        fm_10_runner.runTest(0);
        // mnist
        HNSWLibExclusiveUPRunner m_10_runner(max_iterations, executor_threads, true, false);
        m_10_runner.runTest(1);
        // sift
        HNSWLibExclusiveUPRunner s_10_runner(max_iterations, executor_threads, true, false);
        s_10_runner.runTest(2);
        // gist
        HNSWLibExclusiveUPRunner g_10_runner(max_iterations, executor_threads, true, false);
        g_10_runner.runTest(3);

        alg_config = "11";
        // fashion_mnist
        HNSWLibExclusiveUPRunner fm_11_runner(max_iterations, executor_threads, true, true);
        fm_11_runner.runTest(0);
        // mnist
        HNSWLibExclusiveUPRunner m_11_runner(max_iterations, executor_threads, true, true);
        m_11_runner.runTest(1);
        // sift
        HNSWLibExclusiveUPRunner s_11_runner(max_iterations, executor_threads, true, true);
        s_11_runner.runTest(2);
        // gist
        HNSWLibExclusiveUPRunner g_11_runner(max_iterations, executor_threads, true, true);
