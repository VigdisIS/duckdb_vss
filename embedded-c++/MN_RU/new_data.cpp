    ...
    void runTest(int datasetIdx) {
        try {
            ...
            for (int iteration = 1; iteration <= max_iterations; iteration++) {
                ...
                size_t added = HNSWLibIndexOperations::parallelAddMNRU(index, new_vectors, new_indices,  dataset.name, 
            ...
            // dir name: MN_RU/results/{experiment}/{dataset_name}_{num_queries}q_{num_iterations}i_{partition_size}p/
            std::string output_dir = "MN_RU/results/newdata/" +  experiment + dataset.name + "_" + std::to_string(test_vectors_count) + "q_" + std::to_string(max_iterations) + "i_" + std::to_string((int) (dataset_cardinality/partitions.size())) + "r/";
