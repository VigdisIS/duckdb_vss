...
void RegisterBenchmarks() {
    ...
    for (int tableIndex = 0; tableIndex <= 1; ++tableIndex) {
        for (int cluster_amount : cluster_amounts) {
            benchmark::RegisterBenchmark("BM_ClusteringIndexCreation", BM_ClusteringIndexCreation)->Repetitions(tableIndex < 2 ? 5 : 3) // last two are FOUL😭 I simply do not have the time
