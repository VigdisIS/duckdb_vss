...
HNSWLibFullCoverageRunner(int iterations, int threads, bool use_neigh_update, bool include_tombstones) : db(nullptr), con(db), max_iterations(iterations), threads(threads), use_neigh_update(use_neigh_update), include_tombstones(include_tombstones) {
    ...
    void runTest(int datasetIdx) {
        try {
            ...
            // dir name: MN_RBC/results/{alg_config}/fullcoverage/{experiment}/{dataset_name}_{num_queries}q_{num_iterations}i_{partition_size}p/
            std::string output_dir = "MN_RBC/results/" + alg_config + "/fullcoverage/" + experiment + dataset.name + "_" + std::to_string(test_vectors_count) + "q_" + std::to_string(max_iterations) + "i_" + std::to_string((int) (dataset_cardinality/partitions.size())) + "r/";
