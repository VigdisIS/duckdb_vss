...
USearchNewDataRunner(int iterations, int threads) : db(nullptr), con(db), max_iterations(iterations), threads(threads) {
    ...
    void runTest(int datasetIdx) {
        try {
            ...
            IndexOperations::parallelRunTestQueries(con, index, dataset.name, test_vectors_gt, appender, search_bm_appender, early_termination_appender, 0);
            ...
            for (int iteration = 1; iteration <= 10; iteration++) {
                ...
                IndexOperations::parallelRunTestQueries(con, index, dataset.name, test_vectors_gt_it, appender, 
