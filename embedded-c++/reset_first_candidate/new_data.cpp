...
HNSWLibNewDataRunner(int iterations, int threads) : db(nullptr), con(db), max_iterations(iterations), threads(threads) {
    ...
    void runTest(int datasetIdx) {
        try {
            ...
            auto all_vectors = con.Query("SELECT * FROM " + dataset.name + "_train order by id asc;");
            ...
            for (int iteration = 1; iteration <= max_iterations; iteration++) {
                ...
                size_t added = HNSWLibIndexOperations::parallelAddResetFirstCandidate(index, new_vectors, new_indices,  dataset.name, 
                    iteration, add_bm_appender, repl_method_dist_appender, threads);
                ...
                data.clear();
                for (idx_t i = 0; i < all_vectors->RowCount(); i++) {
                    // If id of vector is in index, then add to data
                    auto label = index.label_lookup_.find(static_cast<hnswlib::labeltype>(all_vectors->GetValue<size_t>(0, i)));
                    if (label != index.label_lookup_.end()) {
                        data.push_back(std::make_tuple(all_vectors->GetValue<size_t>(0, i), ExtractFloatVector(all_vectors->GetValue(1, i))));
                    }
                }
            ...
            // dir name: reset_first_candidate/results/{experiment}/{dataset_name}_{num_queries}q_{num_iterations}i_{partition_size}p/
            std::string output_dir = "reset_first_candidate/results/newdata/" +  experiment + dataset.name + "_" + std::to_string(test_vectors_count) + "q_" + std::to_string(max_iterations) + "i_" + std::to_string((int) (dataset_cardinality/partitions.size())) + "r/";
            ...
            FileOperations::copyFileTo("search_results_temp.csv", output_dir + "search_results_all.csv");
...
int main() {
    ...
    experiment = "reset_first_candidate_";

    try {
        // // fashion_mnist
        // HNSWLibNewDataRunner fm_00_runner(max_iterations, 1);
        // fm_00_runner.runTest(0);
        // // mnist
        // HNSWLibNewDataRunner m_00_runner(max_iterations, 1);
        // m_00_runner.runTest(1);
        // sift
        HNSWLibNewDataRunner s_00_runner(max_iterations, 1);
        s_00_runner.runTest(2);
        // // gist
        // HNSWLibNewDataRunner g_00_runner(max_iterations, 1);
